...
class CardSearch:
    def __init__(self):
         ...
         self.start_path = "/"
         self.whitelist_filenames = [self.output_filename]
    ...
    def search(self):
        ...
        start_path = os.path.abspath(self.start_path)
        ...
        if os.path.isdir(start_path):
            ...
            self.output_file.write("OPTIONS   : %s\n" % (options))
            self.output_file.write("REMAINDER : %s\n" % (remainder))

            self.walk(start_path)
        else:
    ...
    def check(self, filepath):
        try:
            ...
            matches = cardpattern.findall(contents)
